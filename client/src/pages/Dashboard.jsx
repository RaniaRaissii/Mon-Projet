...
// function to log out for candidate
const logout = () =>{
...
useEffect(()=>{
  ...
  return (
    <div className='min-h-screen'>
        ...
        {/* Navbar for candidate panel (recruiter in job portal)*/}
                        ...
                        <div className='relative group'>
                            ...
                            <div className='absolute hidden group-hover:block top-0 right-0 z-10 text-black rounded pt-12'>
                                <ul className='list-none m-0 p-2 bg-white rounded-md border text-sm'>
                                    <li onClick={logout} className='py-1 px-2 cursor-pointer pr-10 whitespace-nowrap'>Log Out</li>
                                </ul>
                            </div>
                        </div>
                    </div>
    ...
    <div className='flex items-start'>
        {/* left sidebar to add resumes,manage resumes,(view applications in insider jobs)*/}
        <div className='inline-block min-h-screen border-r-2'>
            <ul className='flex flex-col items-start pt-5 text-gray-800'>
                <NavLink className={({isActive})=> `flex items-center p-3 sm:px-6 gap-2 w-full hover:bg-gray-100 ${isActive && 'bg-blue-100 border-r-4 border-blue-500'}`} to={'/dashboard/add-job'}>
                    <img className='min-w-4' src={assets.add_icon} alt="" />
                    <p className='max-sm:hidden'>Add Resume</p>
                </NavLink>
                <NavLink className={({isActive})=> `flex items-center p-3 sm:px-6 gap-2 w-full hover:bg-gray-100 ${isActive && 'bg-blue-100 border-r-4 border-blue-500'}`} to={'/dashboard/manage-jobs'}>
                    ...
                    <p className='max-sm:hidden'>Manage Resumes</p>
                </NavLink>
                <NavLink className={({isActive})=> `flex items-center p-3 sm:px-6 gap-2 w-full hover:bg-gray-100 ${isActive && 'bg-blue-100 border-r-4 border-blue-500'}`} to={'/dashboard/view-applications'}>
                    ...
                    <p className='max-sm:hidden'>View Applications (not implemented)</p>
                </NavLink>
            </ul>
        </div>
